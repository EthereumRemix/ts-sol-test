...
async function setupRunEnv (): Promise<void> {
  ...
  if (isYarnRepo) {
    await cli.exec('yarn', ['add', 'tslib', 'mocha', '@remix-project/ghaction-helper@0.1.7-alpha.13', '--dev'])
  } else if (isNPMrepo) {
    await cli.exec('npm', ['install', 'tslib', 'mocha', '@remix-project/ghaction-helper@0.1.7-alpha.13', '--save-dev'])
  } else {
